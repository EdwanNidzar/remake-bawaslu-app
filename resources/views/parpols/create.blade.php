    ...
    <form action="{{ route('parpols.update') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-1">
        <div>
          ...
          <input type="number" name="parpol_number" id="parpol_number" autocomplete="off"
        ...
        <div>
          ...
          <input type="text" name="parpol_name" id="parpol_name" autocomplete="off"
            class="block w-full px-3 py-2 mt-1 text-sm border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
          @error('parpol_name')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
          @enderror
        ...
        <div>
          ...
          <input name="parpol_picture" type="file" class="file-input file-input-bordered w-full max-w-xs"
