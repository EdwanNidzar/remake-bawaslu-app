<x-app-layout>
  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    <form action="{{ route('pelanggarans.update', $pelanggaran->id) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2" x-data="fileHandler()">
        ...
        <div>
          ...
          <select id="parpol_id" name="parpol_id"
            ...
            @foreach ($parpol as $party)
              <option value="{{ $party->id }}" {{ $pelanggaran->parpols_id == $party->id ? 'selected' : '' }}>
        ...
        <div>
          ...
          <select id="jenis_pelanggaran_id" name="jenis_pelanggaran_id"
            ...
            @foreach ($jenispelanggaran as $jp)
              <option value="{{ $jp->id }}" {{ $pelanggaran->jenis_pelanggaran_id == $jp->id ? 'selected' : '' }}>
        ...
        <div>
          ...
          <select id="status_peserta_pemilu" name="status_peserta_pemilu"
            ...
            <option value="DPR RI" {{ $pelanggaran->status_peserta_pemilu == 'DPR RI' ? 'selected' : '' }}>DPR RI
            </option>
            <option value="DPRD Provinsi"
              {{ $pelanggaran->status_peserta_pemilu == 'DPRD Provinsi' ? 'selected' : '' }}>DPRD Provinsi</option>
            <option value="DPRD Kabupaten/Kota"
              {{ $pelanggaran->status_peserta_pemilu == 'DPRD Kabupaten/Kota' ? 'selected' : '' }}>DPRD Kabupaten/Kota
            </option>
            <option value="DPD RI" {{ $pelanggaran->status_peserta_pemilu == 'DPD RI' ? 'selected' : '' }}>DPD RI
            </option>
          </select>
        ...
        <div>
          ...
          <input type="text" name="nama_bacaleg" id="nama_bacaleg" value="{{ $pelanggaran->nama_bacaleg }}"
            class="block w-full px-3 py-2 mt-1 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
        ...
        <div>
          ...
          <select id="dapil" name="dapil"
            class="block w-full px-3 py-2 mt-1 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            <option value="disable" disabled>Pilih Dapil</option>
        ...
        <div>
          ...
          <input type="date" name="tanggal_input" id="tanggal_input" value="{{ $pelanggaran->tanggal_input }}"
            class="block w-full px-3 py-2 mt-1 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
        ...
        <div class="col-span-2">
          ...
          <div class="mt-2 grid grid-cols-3 gap-3">
            @if ($pelanggaran->pelanggaranImages)
              @foreach ($pelanggaran->pelanggaranImages as $image)
                <div class="relative">
                  <img src="{{ Storage::url('pelanggarans/' . $image->image) }}" class="h-50 w-50 object-cover">
                </div>
              @endforeach
            @endif
          </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      statusPesertaPemiluSelect.addEventListener('change', function() {
        ...
        const currentValue = '{{ $pelanggaran->dapil }}';
        dapilSelect.value = currentValue;
      });

      // Trigger change event on load to set the initial state
      statusPesertaPemiluSelect.dispatchEvent(new Event('change'));
    });
  </script>
